...
<Tabs>
  ...
  <Info> 해당 기능은 노티플라이 Android SDK 1.5.0 이상 버전부터 지원됩니다. </Info>
...
<Tab title="Flutter">
  ...
  <Info> 해당 기능은 노티플라이 Flutter SDK 1.4.0 이상 버전부터 지원됩니다.</Info>

    노티플라이 Flutter SDK에서는 푸시 알림 클릭 이벤트를 구독하여 유저가 푸시 알림을 클릭했을 때 앱 내에서 특정 동작을 수행하도록 커스터마이즈 할 수 있습니다. 아래 예시는 Go Router 라이브러리를 사용하여 푸시 알림 클릭 이벤트를 구독하고, 푸시 알림 클릭 시 특정 페이지로 이동하는 예시입니다.
...
<Tabs>
  ...
  `INotificationClick` 의 `notification` 객체에 접근하여, 유저가 어떠한 푸시를 클릭했는지에 대한 정보를 확인할 수 있습니다.
    ...
    <Expandable title="INotification Properties">
        ...
        노티플라이 서버에서 임의로 생성한 메시지 ID입니다. (Since 1.5.0)
        </ResponseField>
...
<Tab title="Flutter">
  ...
  `OSNotificationClickEvent`의 `notification` 멤버에 접근하여, 유저가 클릭한 푸시에 대한 정보를 확인할 수 있습니다.
...
class OSNotificationClickEvent {
  OSNotification notification;
    ...
    <Expandable title="OSNotification Properties">
        ...
        <ResponseField name="campaignId" type="String?">
        푸시 알림이 발송된 노티플라이 캠페인의 ID입니다. (Since 1.4.0)
        </ResponseField>
        ...
        <ResponseField name="notiflyMessageId" type="String?">
        노티플라이 서버에서 임의로 생성한 메시지 ID입니다. (Since 1.4.0)
        </ResponseField>
...
`OSNotificationClickEvent`의 `notification` 멤버에 접근하여, 유저가 클릭한 푸시 알림의 정보를 확인할 수 있습니다.
...
아래는 `OSNotification` 객체의 속성들에 대한 설명입니다.
    ...
    <Expandable title="OSNotification Properties">
        <ResponseField name="androidNotificationId" type="number">
        Android system-level 푸시 알림 ID입니다. (Since 1.4.0)
        </ResponseField>
        ...
        <ResponseField name="customData" type="{[key: string]: any}?">
        FCM 푸시 메시지의 `data` 필드에 포함된 키-값 쌍의 커스텀 데이터입니다. 노티플라이 콘솔에서 설정할 수 있습니다. (Since 1.4.0)
        </ResponseField>
        ...
        <ResponseField name="importance" type="OSNotificationImportance">
        `OSNotificationImportance.HIGH`, `OSNotificationImportance.NORMAL`, `OSNotificationImportance.LOW` 중 하나의 값입니다. (Since 1.4.0)
        </ResponseField>
        ...
        <ResponseField name="rawPayload" type="{[key: string]: any}?">
        FCM data의 가공되지 않은 payload입니다. JSON을 stringify한 형태로 반환됩니다. (Since 1.4.0)
        </ResponseField>

        <ResponseField name="sentTime" type="number">
        FCM 서버에서 푸시 알림을 발송한 시간입니다. (Since 1.4.0)
        </ResponseField>
