...
<Tabs>
<Tab title="Android">
노티플라이 Android SDK는 인앱 팝업(In-App Message) 를 표시하기 위해 내부적으로 WebView를 사용합니다.
...
노티플라이 Android SDK 1.8.0 미만 버전에서는
인앱 팝업 Activity가 시작/종료될 때 SDK 내부에서 `pauseTimers()`와 `resumeTimers()`를 자동으로 호출했습니다.

다음은 해당 버전에서 인앱 팝업를 표시하는 Activity의 예시 코드입니다:

```kotlin highlight={11, 16}
...
이전 버전(1.8.0 미만)의 경우, 인앱 팝업가 닫힌 직후 `pauseTimers()`가 호출되면 앱 내의 다른 WebView가 함께 정지되어 동작하지 않는 문제가 발생할 수 있습니다.
...
인앱 팝업가 정상적으로 표시되지 않는다면, 해당 시점에 WebView가 정상적으로 동작 중인지(즉, `pauseTimers()`로 인해 정지 상태가 아닌지)를 반드시 확인해야 합니다.
