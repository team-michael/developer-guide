...
| `cafe24__shipping_in_progress` | `cafe24__order_completed`의 모든 패러미터`shipping_type`: 배송 유형(국내, 해외)`wished_delivery_date`: 희망배송일(yyyy-MM-dd)`store_pickup`: 매장수령여부(Bool)`shipping_message`: 배송 메세지(Text)                                                                                                                                                                                                           | 배송이 시작되었을 때         | X              |                     |
