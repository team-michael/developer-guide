...
<Note>
  노티플라이의 Connected Content 기능을 사용하면, 실시간 데이터 또는 노티플라이에서 직접 수집하지 않은 외부 데이터를 메시지에 쉽게 통합할 수 있습니다. 이 기능을 통해 각 유저에게 맞춤화된 메시지를 제공함으로써 유저 경험을 향상시킬 수 있습니다.
</Note>
...
노티플라이의 Connected Content 기능은 실시간으로 외부 API를 호출하여 받은 데이터를 메시지에 바로 적용할 수 있게 해주는 기능입니다. 이를 통해 최신 정보를 기반으로 유저 개인에게 최적화된 데이터를 메시지로 전송할 수 있습니다.

### 주요 활용 사례

1. **개인화된 제품 추천**: 유저의 과거 행동 데이터를 기반으로 개인화된 상품을 추천할 때
2. **민감한 데이터 전송**: 노티플라이 서버에 저장하지 않고 실시간으로 민감한 정보를 메시지에 포함할 때
3. **실시간 정보 제공**: 유저의 포인트, 잔액, 주식 가격 등 실시간 정보를 메시지에 포함할 때
...
6. **:basic\_auth**
   ...
   * 기본 인증을 위한 유저 이름과 비밀번호를 포함합니다.
   * 사용법: `:basic_auth "username:password"`
   * 예시: `{% connected_content https://api.example.com/secure :basic_auth "user:pass" %}`

### 에러 처리 및 성능 최적화

* **에러 처리**: Connected Content 호출 시 URL이 잘못되었거나 서버에서 4xx, 5xx 응답을 반환할 경우, 기본적으로 메시지가 중단됩니다. 이때, 유저에게 메시지를 전송하지 않도록 `{% abort_message() %}` 태그를 사용할 수 있습니다.
