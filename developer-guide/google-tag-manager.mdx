---
title: "구글 태그 매니저 설정하기"
---

Google Tag Manager(GTM)에 노티플라이 맞춤 템플릿을 추가해 코드 수정 없이 손쉽게 SDK 초기화, 유저/이벤트 연동을 설정할 수 있습니다.

## 1. 노티플라이 맞춤 템플릿 설치

1. 최신 템플릿 다운로드
[릴리스 페이지](https://github.com/notifly-tech/notifly-gtm-template/releases)에서 템플릿을 내려받습니다.
   - 한글: **template.ko.tpl**
   - 영문: **template.tpl**
...
2. 태그 이름 지정 → 태그 구성 클릭
3. 맞춤 템플릿 섹션에서 노티플라이 JavaScript SDK 선택
   ![맞춤 설정](./image/gtm/gtm-init-1.png)
4. 태그 구성에서 Tag Type: SDK 초기화 선택 후 Project ID, Username, Password 입력. ([노티플라이 SDK 초기화 옵션](/developer-guide/javascript-sdk#2-sdk-%EC%B4%88%EA%B8%B0%ED%99%94) 참고.)
...
유저 행동(클릭, 조회, 구매 등)을 이벤트로 전송해 세그먼트/트리거/성과 분석에 활용합니다.
<Info> 
...
- [JavaScript SDK 가이드의 웹 푸시 설정 섹션](/developer-guide/javascript-sdk#1-%EA%B8%B0%EB%B3%B8-%EC%84%A4%EC%A0%95)을 참고하여 사이트에 `notifly-service-worker.js` 파일을 추가해주세요.
