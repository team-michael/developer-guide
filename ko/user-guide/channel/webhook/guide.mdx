...
웹훅은 특정 조건이 충족되면 한 시스템에서 다른 시스템으로 어떠한 메시지를 보내는 것입니다. 노티플라이에서는 이 조건이 일반적으로 다음과 같습니다.

* 유저의 이벤트 발송
* 예약 발송 (주기 발송) 시 예약된 시점 도달

이러한 조건이 충족되면, 노티플라이에서는 HTTP 요청을 통해 명시된 URL, HTTP method, HTTP 헤더, HTTP 본문을 전송합니다.
...
1. 웹훅을 수신할 수 있는 환경
   ...
   * 알맞은 DNS 설정을 통해, 명시된 URL으로 노티플라이 서버가 해당 서버에 도달할 수 있어야 합니다.

2. 노티플라이 IP Whitelist 설정

   * 노티플라이 서버의 IP 주소로부터의 Inbound Traffic을 네트워크 정책에서 꼭 허용하도록 설정해주세요. 노티플라이 웹훅 발송 서버의 IP 주소의 목록은 다음과 같습니다.
...
웹훅 캠페인은 개인화와 함께 사용하셔야 더욱 의미있는 결과를 얻을 수 있습니다. 웹훅 캠페인을 생성하기 위해서는 다음과 같은 정보를 입력해야 합니다. 메시지 개인화에 대한 자세한 정보는 [메시지 개인화](/ko/user-guide/campaigns/personalization/basic) 섹션을 참고해주세요.

![Build Webhook Campaign - 1](/ko/user-guide/images/user-guide/webhook/ko/assets/images/webhook-message-1-bc786b2acf0948f309a75a3e2f9e7dae.png)
...
노티플라이는 메시지 개인화를 위해 [Liquid Template 언어](https://liquidjs.com/tutorials/intro-to-liquid.html)를 사용합니다. 아래 웹훅 Body 예시에서는 유저의 프로퍼티를 이용하여 메시지를 개인화합니다.
...
<Check>
  ```
  {%- assign additional_properties = user["properties"] -%}{%- if additional_properties -%}{  "name": {{ user["name"] | default: "" | json }},  "email": {{ user["properties"] | json }}}{%- else -%}{  "name": {{ user["name"] | default: "" | json }},  "properties": null}{%- endif -%}
  ```
...
![Test Webhook Campaign - 1](/ko/user-guide/images/user-guide/webhook/ko/assets/images/webhook-test-1-5aead313d711a7c244ae2432d3b61c66.png)

* 발송할 유저 아이디를 입력하신 후, '테스트 발송'을 클릭해주세요.

![Test Webhook Campaign - 2](/ko/user-guide/images/user-guide/webhook/ko/assets/images/webhook-test-2-f5eca992d45dfacf6e0a38198116b220.png)

* 발송 결과를 탭하시면, 더 자세한 정보를 확인하실 수 있습니다.

![Test Webhook Campaign - 3](/ko/user-guide/images/user-guide/webhook/ko/assets/images/webhook-test-3-67a5c6146f231c8169786f5d94050835.png)
